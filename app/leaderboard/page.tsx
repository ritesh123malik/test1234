            ...
            <header className='flex flex-col md:flex-row md:items-end justify-between gap-8'>
                <div className="space-y-3">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className='text-white text-5xl font-black tracking-tighter'>Arena</h1>
                    </div>
                    <p className='text-gray-400 text-lg max-w-lg'>Compete with top students across India. Based on total XP from interviews and daily challenges.</p>
                </div>

                <div className="bg-gray-900 border border-gray-800 p-2 rounded-3xl flex gap-1 shadow-2xl">
                    {tabs.map(tab => (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                            className={`px-6 py-2.5 rounded-2xl text-sm font-black uppercase tracking-widest transition-all duration-300 flex items-center gap-2 ${activeTab === tab.id
